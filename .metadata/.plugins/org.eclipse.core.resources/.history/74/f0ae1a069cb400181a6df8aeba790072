...
public class DetailPCCtrlActivity extends TitledActivity
		implements
			OnClickListener {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_ONLINE)) {
				if (true == NotifyProcessor.onlineNotify(
						DetailPCCtrlActivity.this, intent)) {
					...
					if (m_PCIDList.indexOf(plugid) != -1) {
						for (int i = 0; i < m_QuestionsList.size(); i++) {
							...
							if (pcID.equals(plugid) == true) {
								if (online == 1) {
									m_QuestionsList.get(i).setTv_pc_online(
											"已开机");
								} else {
									m_QuestionsList.get(i).setTv_pc_online(
											"已关机");
	...
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState, R.layout.activity_detail_pcctrl,
				false);
		...
		tv_count.setText(String.valueOf(macs.size()) + "个");

		question_dapter = new QuestionListAdapter(this, m_QuestionsList,
				mHandler);
		lv_pc_macs.setAdapter(question_dapter);
		...
		switch (view.getId()) {
			...
			case R.id.titlebar_rightbutton :
				...
				if (btn_TitleRight.getText().equals(
					...
					// intent.setClass(DetailPCCtrlActivity.this,
			...
			case R.id.tv_pcselect :
				...
				for (int i = 0; i < m_QuestionsList.size(); i++) {
					m_QuestionsList.get(i).setCb_pc_mac(b_pcselect_all);
				}
				mHandler.sendEmptyMessage(0);
	...
	private ArrayList<String> getPCModuleMac() {
		ArrayList<String> macs_show = mPlugHelper
				.getAllSmartPlugMacShow(mPlugId);
		return macs_show;
	}
	...
	public class Question {
		...
		public void setTv_pc_no(String pc_no) {
			this.pc_no = pc_no;
		}
	...
	public class QuestionListAdapter extends BaseAdapter {
		...
		private class ViewHolder {
			...
			public RelativeLayout rl_mac_item;

			public void ViewData(Question news, int position) {
				tv_pc_no.setText(news.getTv_pc_no());
				...
				tv_pc_online.setVisibility(View.VISIBLE);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(R.layout.item_pcctrl_mac_list,
						null);
				holder.rl_mac_item = (RelativeLayout) convertView
						.findViewById(R.id.rl_mac_item);
				...
				holder.cb_pc_mac = (ThingzdoCheckBox) convertView
						.findViewById(R.id.cb_pc_mac);
						...
						.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View v) {
								boolean b_result = !mNewsList.get(t_position)
										.isCb_pc_mac();
								mNewsList.get(t_position)
										.setCb_pc_mac(b_result);
								mHandler.sendEmptyMessage(0);
							}
						});
