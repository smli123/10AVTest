...
public class SmartPlugEventHandlerEnableIRScene extends SmartPlugEventHandler {
	Intent mIntent = new Intent(PubDefine.PLUG_AIRCON_ENABLEIRSCENE_ACTION);
	...
	public void handleMessage(Message msg) {
		...
		try {
			...
			if (0 == ret) {
				...
				if (0 != resid) {
					mIntent.putExtra("MESSAGE", SmartPlugApplication
							.getContext().getString(resid));
				} else {
					mIntent.putExtra(
							"MESSAGE",
							SmartPlugApplication.getContext().getString(
									R.string.smartplug_ctrl_settimer_fail));
				}
