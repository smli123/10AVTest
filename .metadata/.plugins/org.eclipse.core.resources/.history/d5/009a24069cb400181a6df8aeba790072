 ...
 * 
 */
public class PlugTimerActivity extends TitledActivity
		implements
			OnClickListener {
	...
	private void addTimer() {
		if (mTimerType == PubDefine.TIMER_TYPE_OPENPC
				|| mTimerType == PubDefine.TIMER_TYPE_CLOSEPC) { // 1_2
			for (int i = 0; i < selectNOs.size(); i++) {
				...
				if (mTimerType == PubDefine.TIMER_TYPE_CLOSEPC
						&& mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG) { // 1_2
					tempModuleID = mPlugHelper.getPlugIDFromMac(mPlugId,
							macs.get(selectNOs.get(i)));
				}

				mTimertask.mPlugId = tempModuleID;
				mTimertask.mPowerOnTime = mPoweronTime;
				if (mTimerType == PubDefine.TIMER_TYPE_OPENPC) {
					mTimertask.mPowerOffTime = macs.get(selectNOs.get(i));
				} else {
					mTimertask.mPowerOffTime = mPoweroffTime;
				}
				mTimertask.mTimerId = mMaxTimerId + i;
	...
	private void modifyTimer() {
		...
		if (mTimerType == PubDefine.TIMER_TYPE_CLOSEPC
				&& mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG) { // 1_2)
																			// {
			tempModuleID = mPlugHelper.getPlugIDFromMac(mPlugId, et_mac
					.getText().toString());
		}
	...
	private void addTimerTask() {
		...
		String str_temp = (mTimerType == PubDefine.TIMER_TYPE_OPENPC)
				? str_mac
				: mPoweroffTime;
		...
		// if (mTimerType == PubDefine.TIMER_TYPE_CLOSEPC
		// && mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG) { // 1_2
		// tempModuleID = mPlugHelper.getPlugIDFromMac(mPlugId, str_mac);
		// }

		if (mTimerType == PubDefine.TIMER_TYPE_OPENPC) {
			// 存在下发多个Mac地址的问题；
			i_count = selectNOs.size();
			for (int i = 0; i < selectNOs.size(); i++) {
				str_temp = macs.get(selectNOs.get(i));
		...
		} else if (mTimerType == PubDefine.TIMER_TYPE_CLOSEPC) {
			// 存在下发多个Mac地址的问题；
			i_count = selectNOs.size();
			for (int i = 0; i < selectNOs.size(); i++) {
				str_temp = macs.get(selectNOs.get(i));
				tempModuleID = mPlugHelper.getPlugIDFromMac(mPlugId, str_temp);
	...
	private void modifyTimerTask() {
		...
		if (mTimerType == PubDefine.TIMER_TYPE_OPENPC) {
			str_temp = macs.get(selectNOs.get(0));
		}
		...
		sb.append(SmartPlugMessage.CMD_SP_MODTIMER)
				...
				.append(StringUtils.PACKAGE_RET_SPLIT_SYMBOL)
				.append(mPoweronTime)
				.append(StringUtils.PACKAGE_RET_SPLIT_SYMBOL).append(str_temp);
