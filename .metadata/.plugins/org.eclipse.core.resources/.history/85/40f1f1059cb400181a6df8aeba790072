...
public class SmartPlugEventHandlerGetWeight extends SmartPlugEventHandler {
	Intent mIntent = new Intent(PubDefine.PLUG_STEELYARD_GETWEIGHT_ACTION);
	@Override
	public void handleMessage(Message msg) {
		...
		try {
			...
			double status = Double
					.parseDouble(buffer[EVENT_MESSAGE_HEADER + 1]);
