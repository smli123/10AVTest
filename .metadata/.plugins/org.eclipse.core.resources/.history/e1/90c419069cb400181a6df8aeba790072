...
public class DetailKettleAddWaterActivity extends TitledActivity
		implements
			OnClickListener {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_ONLINE)) {
				if (true == NotifyProcessor.onlineNotify(
						DetailKettleAddWaterActivity.this, intent)) {
					updateUI();
				}
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState,
				R.layout.activity_plug_detail_kettle_addwater, false);
		...
		switch (view.getId()) {
			...
			case R.id.titlebar_rightbutton :
				...
				if (btn_TitleRight.getText().equals(
					...
					intent.setClass(DetailKettleAddWaterActivity.this,
	...
	private void add_water() {
		...
		mProgress = PubFunc.createProgressDialog(
				DetailKettleAddWaterActivity.this,
				getString(R.string.str_btn_curtainopen), false);
		mProgress.show();

		String command_base = "WATER," + PubStatus.g_CurUserName + ","
				+ mPlugId + "," + "1";
