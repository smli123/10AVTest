...
public class DetailKettleActivity extends TitledActivity
		implements
			OnClickListener,
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_ONLINE)) {
				if (true == NotifyProcessor.onlineNotify(
						DetailKettleActivity.this, intent)) {
					updateUI();
				}
			}
			...
			if (intent.getAction().equals(PubDefine.PLUG_CURTAIN_ACTION)) {
				...
				if (result == 0) {
					// PubFunc.thinzdoToast(DetailKettleActivity.this,
			...
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_KETTLE)) {
				notify_ui(intent);
				// if (true ==
				// NotifyProcessor.curtainNotify(DetailKettleActivity.this,
				// intent)) {
				// updateUI();
				// }
			}
	...
	private void saveInfo() {
		...
		editor.putInt(mPlugId + "_value_cur_warm_temp_pos",
				value_cur_warm_temp_pos);
		...
		editor.putInt(mPlugId + "_value_notify_temperature_pos",
				value_notify_temperature_pos);
		...
		editor.putBoolean(mPlugId + "_value_notify_temperature",
				value_start_notify);
		editor.commit();
	}
	...
	private void loadInfo() {
		...
		value_cur_warm_temp_pos = mCurtainInfo.getInt(mPlugId
				+ "_value_cur_warm_temp_pos", 80);
		value_cur_power_pos = mCurtainInfo.getInt(mPlugId
				+ "_value_cur_power_pos", 400);
		value_left_time_pos = mCurtainInfo.getInt(mPlugId
				+ "_value_left_time_pos", 60);
		value_notify_temperature_pos = mCurtainInfo.getInt(mPlugId
				+ "_value_notify_temperature_pos", 40);
		value_cur_mode = mCurtainInfo.getString(mPlugId + "_value_cur_mode",
				"SHAOSHUI");
		value_start_baowen = mCurtainInfo.getBoolean(mPlugId
				+ "_value_start_baowen", true);
		value_start_notify = mCurtainInfo.getBoolean(mPlugId
				+ "_value_notify_temperature", true);
	}

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState, R.layout.activity_detail_kettle,
				false);
		...
		mCurtainInfo = getSharedPreferences("KettleInfo"
		...
		if (null != sb_cur_warm_temp) {
			...
			tv_cur_warm_temp
					.setText(trans_temperature(value_cur_warm_temp_pos));
		}
		...
		if (null != sb_left_time) {
			...
			tv_notify_temperature
					.setText(trans_temperature(value_notify_temperature_pos));
		}
	...
	public void onProgressChanged(SeekBar bar, int pos, boolean arg2) {
		switch (bar.getId()) {
			...
			case R.id.sb_notify_temperature :
				tv_notify_temperature.setText(trans_temperature(bar
						.getProgress()));
				break;
			default :
				break;
		...
		switch (view.getId()) {
			...
			case R.id.titlebar_rightbutton :
				...
				if (btn_TitleRight.getText().equals(
					...
					intent.setClass(DetailKettleActivity.this,
	...
	private void notify_ui(Intent intent) {
		...
		value_cur_mode = intent.getStringExtra("CUR_MODE"); // 当前模式;
															// SHAOSHUI,SUANNAI,HUACHA
		if (value_cur_mode.equals("SHAOSHUI")) {
			tv_current_mode.setText("烧水");
		} else if (value_cur_mode.equals("SUANNAI")) {
			tv_current_mode.setText("酸奶");
		...
		if (value == 1) {
			if (value_cur_temp <= value_notify_temperature_pos
					&& (value_cur_temp >= (value_notify_temperature_pos - 2))) {
				notify_mp3();
			}
		}
	}
	...
	private void kettle_open() {
		...
		mProgress = PubFunc.createProgressDialog(DetailKettleActivity.this,
				getString(R.string.str_btn_curtainopen), false);
		mProgress.show();

		String command_base = "SHUIHU," + PubStatus.g_CurUserName + ","
				+ mPlugId + "," + "1," + String.valueOf(value_cur_power_pos)
				+ ",HUACHA";
	...
	private void kettle_baowen() {
		...
		String command_base = "BAOWEN," + PubStatus.g_CurUserName + ","
				+ mPlugId + "," + checked + ","
				+ String.valueOf(value_cur_warm_temp_pos) + ","
				+ String.valueOf(value_left_time_pos);
		apppassthrought_command(command_base);
	}
	...
	private void kettle_add_water() {
		...
		String command_base = "DELEY_W," + PubStatus.g_CurUserName + ","
				+ new_moduleid + "," + "1,3000";
