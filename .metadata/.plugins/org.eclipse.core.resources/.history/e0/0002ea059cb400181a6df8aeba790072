...
public class SmartPlugEventHandlerDeleteIRScene extends SmartPlugEventHandler {
	Intent mIntent = new Intent(PubDefine.PLUG_AIRCON_DELIRSCENE_ACTION);
		...
		if (0 == ret) {
			...
			if (0 == resid) {
				mIntent.putExtra("MESSAGE", SmartPlugApplication.getContext()
						.getString(R.string.smartplug_ctrl_deletetimer_fail));
			} else {
				mIntent.putExtra("MESSAGE", SmartPlugApplication.getContext()
						.getString(resid));
			}
