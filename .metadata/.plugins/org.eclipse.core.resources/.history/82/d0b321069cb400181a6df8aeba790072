...
public class PlugDetailActivity extends TitledActivity
	...
	private LinearLayout ll_Power2Ctrl;
	...
	private TextView mImgPower2Ctrl = null;
	private boolean mIsPowe2On = false;
	...
	private RelativeLayout mLayoutStatic_bell = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mLayoutStatic_bell = (RelativeLayout) findViewById(R.id.layout_bell_static);
		...
		mLayoutStatic_power2 = (RelativeLayout) findViewById(R.id.layout_power2_static);
		mTxtStatic_power = (TextView) findViewById(R.id.txt_power_static);
		mTxtStatic_light = (TextView) findViewById(R.id.txt_light_static);
		mTxtStatic_bell = (TextView) findViewById(R.id.txt_bell_static);
		...
		mTxtStatic_power2 = (TextView) findViewById(R.id.txt_power2_static);
	...
	private void addView() {
		...
		ll_Power2Ctrl = (LinearLayout) viewCtrl_1
				.findViewById(R.id.ll_Power2Ctrl);
		...
		mImgPower2Ctrl = (TextView) viewCtrl_1.findViewById(R.id.imgPower2Ctrl);
		mImgPower2Ctrl.setOnClickListener(this);

		mLayoutStatic_bell.setVisibility(View.GONE);
		if (mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_SIMULATION_PC
			...
			ll_Power2Ctrl.setVisibility(View.GONE);
			...
			mLayoutStatic_power2.setVisibility(View.GONE);
		...
		// 对于智能电灯控制器（5_2），只显示Power1,Power2
		else if (mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_SWITCH
			...
			ll_UsbCtrl.setVisibility(View.GONE);
			ll_LockCtrl.setVisibility(View.GONE);
			ll_Power2Ctrl.setVisibility(View.VISIBLE);

			layout_light_setting.setVisibility(View.GONE);

			mLayoutStatic_power.setVisibility(View.VISIBLE);
			mLayoutStatic_light.setVisibility(View.GONE);
			mLayoutStatic_bell.setVisibility(View.VISIBLE);
			mLayoutStatic_usb.setVisibility(View.GONE);
			mLayoutStatic_openpc.setVisibility(View.GONE);
			mLayoutStatic_closepc.setVisibility(View.GONE);
			mLayoutStatic_power2.setVisibility(View.VISIBLE);
	...
	private void updateUI() {
		...
		mIsPowe2On = (32 == (mPlug.mDeviceStatus & 32) ? true : false);
		...
		drawable = getResources().getDrawable(
				mIsPowe2On
						? R.drawable.smp_power2_on_big
						: R.drawable.smp_power2_off_big);
		// 对于智能电灯控制器（7_1），只显示Power
		if (mPlug.mSubDeviceType == PubDefine.DEVICE_SMART_SIMULATION_PC
				&& mPlug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 7_1
			drawable = getResources().getDrawable(R.drawable.smp_power2_on_big);
		}

		drawable.setBounds(0, 0, drawable.getIntrinsicWidth(),
				drawable.getIntrinsicHeight());
		mImgPower2Ctrl.setCompoundDrawables(null, drawable, null, null);
		...
		if (PubDefine.SmartPlug_Connect_Mode.WiFi == PubDefine.g_Connect_Mode) {
			...
			if (false == mPlug.mIsOnline) {
				...
				mImgPower2Ctrl.setEnabled(false);
			} else {
				mLayoutTimer.setOnClickListener(addTimerClick);
				mImgPowerCtrl.setEnabled(true);
				mImgLightCtrl.setEnabled(true);
				mImgUsbCtrl.setEnabled(true);
				mImgLockCtrl.setEnabled(true);
				mImgPower2Ctrl.setEnabled(true);
			}
			// 临时调试使用，后续需要删除
			mImgUsbCtrl.setEnabled(true);
			mImgLightCtrl.setEnabled(true);
	...
	private void showTimerTypeDlg() {
		...
		// 对于智能电灯控制器（5_2），显示Power1和Power2
			...
			dlg.addSheetItem(PubFunc.getTimerTypeLabel(PlugDetailActivity.this,
					PubDefine.TIMER_TYPE_POWER2), SheetItemColor.Blue,
					new OnSheetItemClickListener() {
						@Override
						public void onClick(int which) {
							Intent intent = new Intent();
			...
			dlg.addSheetItem(PubFunc.getTimerTypeLabel(PlugDetailActivity.this,
					PubDefine.TIMER_TYPE_BELL), SheetItemColor.Blue,
					new OnSheetItemClickListener() {
						@Override
						public void onClick(int which) {
							Intent intent = new Intent();
				...
				&& mPlug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 1_1
	...
	private void initStatics(final ArrayList<TimerDefine> timers) {
		...
		count = getTaskCount(2, timers);
		/*
		 * if (0 == count) { mLayoutStatic_light.setVisibility(View.GONE); }
		 * else {
		 */
		// mLayoutStatic_light.setVisibility(View.VISIBLE);
		mTxtStatic_bell.setText(String.valueOf(count));
		// }
		...
		count = getTaskCount(6, timers);
		mTxtStatic_power2.setText(String.valueOf(count));
	}
	...
	public void init() {
		...
		for (int i = timers.size() - 1; i >= 0; i--) {
			if (timers.get(i).mType == PubDefine.TIMER_TYPE_OPENPC) {
				String pcID = mPlugHelper.getPlugIDFromMac(mPlugId,
						timers.get(i).mPowerOffTime);
				String pcName = mPlugHelper.getSmartPlugMacShowName(pcID);
				// timers.get(i).mPowerOffTime = "[" + pcName + "] "
				// + timers.get(i).mPowerOffTime;
				timers.get(i).mPowerOffTime = timers.get(i).mPowerOffTime;
				timers.get(i).mPCName = "[" + pcName + "]";
			}
	...
	public void onClick(View view) {
		...
		switch (view.getId()) {
			...
			case R.id.imgPowerCtrl :
				...
				break;
			case R.id.imgPower2Ctrl :
				// TestCommand(mPlugId);
				power2Plug(mPlugId, !mIsPowe2On);
				break;
	...
	private void onIRclick() {
		ActionSheetDialog dlg = new ActionSheetDialog(mContext).builder()
				.setCancelable(true).setCanceledOnTouchOutside(true);

		dlg.addSheetItem("空调", SheetItemColor.Blue,
				new OnSheetItemClickListener() {
					@Override
					public void onClick(int which) {
						Intent intent = new Intent();
		...
		if (PubDefine.RELEASE_VERSION == false) { // DEBUG
			dlg.addSheetItem("电视", SheetItemColor.Blue,
					new OnSheetItemClickListener() {
						@Override
						public void onClick(int which) {
							Intent intent = new Intent();
	...
	private void power2Plug(String plugId, boolean power) {
		mErrorMsg = power
				? getString(R.string.smartplug_ctrl_poweron_fail)
				: getString(R.string.smartplug_ctrl_poweroff_fail);
		mProgress = PubFunc.createProgressDialog(
